...
def remove_player(player_id: int, player_data_directory: str, map_directory) -> None:
    ...
    room_data = get_room_data(map_directory, player_data["area"], player_data["room"])

    # Remove the player from the room
    room_data["players"][:] = [x for x in room_data["players"] if x != player_id]

    _save_room_data(map_directory, player_data["area"], player_data["room"], room_data)

def add_player(player_id: int, map_directory: str, area: str, room: str) -> None:
    """Adds a player to the player register of the specified position"""
    # Get the room data
    room_data = get_room_data(map_directory, area, room)

    # Add the player to the room data
    room_data["players"].append(player_id)

    _save_room_data(map_directory, area, room, room_data)

def _save_room_data(map_directory: str, area: str, room: str, room_data: dict) -> None:
    """Save the given room data to a given room in a given area"""
    with open(os.path.join(map_directory, f"{area}.json"), "r") as f:
        area_data = json.load(f)
    area_data[room] = room_data
...
def get_room_data(map_directory: str, area: str, room: str) -> dict:
    ...
    if not os.path.exists(os.path.join(map_directory, f"{area}.json")):
    ...
    area = json.load(open(os.path.join(map_directory, f"{area}.json"), "r"))
